...
| `{{{ slots.footer-info }}}`   | Footer site information slot                                       |
| `{{{ slots.social-list }}}`   | Social link list slot                                              |

Double braces perform HTML escaping and are suitable for text from frontmatter.

Triple braces do not escape HTML. Use them only for trusted HTML generated by the builder, such as `content`, `editorHelp`, `slots.header`, `slots.headerDocNav`, `slots.sidebar`, `slots.mobileSidebar`, `slots.aside`, `slots.prevNext`, `slots.footer-info`, and `slots.social-list`.
...
The built-in `default` layout reuses the common documentation structure: left sidebar, content, and right-side table of contents. Its core template structure is:
